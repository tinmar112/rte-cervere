...
class Cable:
    ...
    def _years_to_unpack(self) -> list[int]:
        """
        ...
        years = [date.year for date in pd.date_range(start, end, freq='YS')]
        return years

    def _read_data(self, chemin: str) -> None:
        """Routine de lecture d'un fichier DTS brut"""
        pass

    def _loop(self) -> None:
        """Passe dans l'ensemble des dossiers contenant les données."""
        for year in self._years_to_unpack():
            for file in os.listdir(self._dossier + '/' + str(year)):
                self._read_data(self._dossier + '/' + str(year) + '/' + file)
    ...
    def _interpolate(self) -> pd.DataFrame:
        ...
        # Concaténer sur toute la longueur
    ...
    def extract(self) -> pd.DataFrame:
        ...
        try :
            ...
            logger.critical(f'Aucune nouvelle trouvée sur cet intervalle: {e}')
